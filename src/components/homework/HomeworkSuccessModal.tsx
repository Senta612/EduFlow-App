...
export interface HomeworkSuccessData {
  title: string;
  batchName: string;
...
export interface HomeworkSuccessModalProps {
  visible: boolean;
  onClose: () => void;
  data: HomeworkSuccessData | null;
  onViewBatch?: () => void;
}

export function HomeworkSuccessModal({
  visible,
  onClose,
  data,
  onViewBatch,
}: HomeworkSuccessModalProps) {
  ...
  const halfDone = data?.halfDoneCount || 0;
  const notDone = data?.notDoneCount || 0;
  const completionRate = total > 0 ? Math.round((done / total) * 100) : 0;
  const isAllDone = total > 0 && notDone === 0 && halfDone === 0;
  ...
  useEffect(() => {
    if (visible) {
      ...
      progressFill.value = withTiming(completionRate, {
        duration: 750,
  ...
  }, [visible, completionRate, pulseScale, pulseOpacity, progressFill]);
        ...
        <Pressable style={styles.backdropPressable} onPress={onClose}>
          ...
          >
            <Pressable onPress={(e) => e.stopPropagation()} style={styles.cardInner}>
              ...
              <View style={styles.statsGrid}>
                ...
                <View style={[styles.statBox, styles.statBoxDone]}>
                  <View style={styles.statHeaderRow}>
                    ...
                    />
                    <Text variant="heading" style={styles.statNumberDone}>
                      {done}
                    </Text>
                  </View>
                  <Text variant="caption" style={styles.statLabelDone}>
                    Completed
                  </Text>
                </View>
                ...
                <View style={[styles.statBox, styles.statBoxPartial]}>
                  <View style={styles.statHeaderRow}>
                    <Feather
                      name="clock"
                      size={12}
                      color={halfDone > 0 ? theme.colors.semantic.warning.main : theme.colors.text.disabled}
                    />
                    <Text
                      variant="heading"
                      style={[
                        styles.statNumberPartial,
                        halfDone === 0 && styles.statNumberDisabled,
                      ]}
                    >
                      {halfDone}
                    </Text>
                  ...
                  <Text
                    variant="caption"
                    style={[
                      styles.statLabelPartial,
                      halfDone === 0 && styles.statLabelDisabled,
                    ]}
                  >
                    Partial
                  </Text>
                ...
                {/* Not Done / Pending */}
                <View style={[styles.statBox, styles.statBoxNotDone]}>
                  <View style={styles.statHeaderRow}>
                    <Feather
                      name="x-circle"
                      size={12}
                      color={notDone > 0 ? theme.colors.semantic.danger.main : theme.colors.text.disabled}
                    />
                    <Text
                      variant="heading"
                      style={[
                        styles.statNumberNotDone,
                        notDone === 0 && styles.statNumberDisabled,
                      ]}
                    >
                      {notDone}
                    </Text>
                  ...
                  <Text
                    variant="caption"
                    style={[
                      styles.statLabelNotDone,
                      notDone === 0 && styles.statLabelDisabled,
                    ]}
                  >
                    Pending
                  </Text>
              ...
              {/* Completion Rate Progress Bar with Dynamic Fill */}
                ...
                <View style={styles.progressBarTrack}>
                  <Animated.View
                    style={[
                      ...
                      completionRate < 50 && styles.progressBarFillLow,
                    ]}
                  />
                </View>
              </View>
